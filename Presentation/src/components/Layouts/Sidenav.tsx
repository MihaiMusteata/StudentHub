  ...
  const navItem = (icon: JSX.Element, text: string, link: string) => {
    return (
      <li className='nav-item'>
        <Link
          to={link} className={`nav-link text-white ${location.pathname.includes(link) ? 'active' +
          ' bg-gradient-primary' : ''}`}
        >
      ...
      <div className='collapse navbar-collapse w-auto' id='sidenav-collapse-main'>
        <ul className='navbar-nav'>
          <li className='nav-item mt-3'>
            <h6 className='ps-4 ms-2 text-uppercase text-xs text-white font-weight-bolder opacity-8'>Account
              Management
            </h6>
          </li>
